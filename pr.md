...
- Stream-level WINDOW_UPDATE with threshold-based batching (50% of initial window) to reduce write stalls
- WINDOW_UPDATE piggyback: encoded after pending data sends for TCP write batching
...
- `flow_controlled_len` tracking for RFC 7540 §6.9.1 padding compliance
- Direct state machine access architecture (like compio-quic) — no channels, `Rc<RefCell<ConnShared>>` for thread-per-core
...
- Zerocopy threshold (8KB) in compio-net: small writes use regular `send` to avoid ~40ms delayed-ACK stall from io_uring zerocopy buffer-release CQE
- Cancel safety documentation on all public async methods
- GOAWAY sent on truncated/invalid connection preface (RFC 9113 §3.4)
...
**After zerocopy threshold fix:**

| Benchmark | compio-h2 | h2 crate | Speedup |
|-----------|----------:|---------:|--------:|
| Handshake | 74 µs | 90 µs | 1.2x |
| Request/response 100B | 14.8 µs (12.9 MiB/s) | 128 µs (1.5 MiB/s) | **8.7x** |
| Request/response 1KB | 14.7 µs (133 MiB/s) | 126 µs (15.5 MiB/s) | **8.6x** |
| Request/response 64KB | 27 µs (4.5 GiB/s) | 182 µs (686 MiB/s) | **6.7x** |

compio-h2 achieves **6–9x lower latency** than the h2 crate across all payload sizes, driven by the direct state machine access architecture (no channel overhead) and completion-based I/O.

**Before zerocopy threshold fix:**

io_uring `send_zerocopy` requires an extra CQE for buffer-release notification. For small writes, this triggered a ~40ms TCP delayed-ACK stall. The fix adds a `ZEROCOPY_THRESHOLD` (8KB) in compio-net — writes below this size use regular `send` instead.
...
The 64KB result (88.6ms vs 27µs) shows the dramatic impact of the zerocopy stall — a **3,200x improvement** after the fix.

---

## Test plan

- [x] `cargo test -p compio-h2` — all tests pass
- [x] `cargo test -p compio-net --test tcp_write_threshold` — zerocopy threshold tests
- [x] `cargo check -p compio-h2 --features native-tls` — TLS with native-tls
- [x] `cargo check -p compio-h2 --features rustls` — TLS with rustls
- [x] `cargo check -p compio-h2 --no-default-features` — builds without TLS
- [x] `cargo doc -p compio-h2 --no-deps` — docs build clean
- [x] `cargo clippy -p compio-h2` — no warnings (1 expected RefCell-across-await)
- [x] h2spec conformance: 147/147 passed in strict mode
- [x] `cargo bench -p compio-h2 --bench rpc` — benchmarks run successfully
