#!/usr/bin/env bash
# Run compio-h2 tests with AddressSanitizer (ASan) + LeakSanitizer (LSan) to
# detect memory leaks, use-after-free, buffer overflows, and stack overflows.
#
# Unlike Miri, ASan works with real I/O (io_uring, TCP) so this covers
# integration tests and the full async runtime path.
#
# Requires: nightly toolchain with rust-src component
#   rustup component add rust-src --toolchain nightly
#
# Usage:
#   ./scripts/memleak/asan.sh                      # run all compio-h2 tests
#   ./scripts/memleak/asan.sh -- roundtrip         # filter by test name
...
EXTRA_ARGS=(-p compio-h2 "$@")

echo "=== ASan + LSan: detecting memory errors and leaks (compio-h2) ==="
echo "Target: $TARGET"
