...
| **Miri** | Rust (nightly) | Undefined behavior and leak detection on pure-logic modules |
| **ASan** | Rust (nightly) | AddressSanitizer + LeakSanitizer on real I/O tests |
...
└── scripts/
    ...
    └── memleak/                # Memory safety testing
        ├── miri.sh             # Miri on pure-logic modules
        ├── asan.sh             # ASan + LSan on real I/O tests
